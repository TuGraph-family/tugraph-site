export default {
  ...
  'home.banner.demonstrationCase': '演示案例',
  ...
  'home.reason.desc0':
    '源自蚂蚁集团的数据计算和分析要求，让客户能够轻松满足合规、审计、访问控制等要求。具备金融级高可用性、可串行化隔离级别，超越灾备标准最高级（6级），IT团队可以实现业务系统不间断运行，保护数据不丢失。',
  'home.reason1': '优化企业业务性能',
  'home.reason.desc1':
    'HTAP图数据库，满足实时环境（实时审核或推荐）和离线分析（如审计）的毫秒级复杂查询需求，一站式存储、查询、分析。具备万亿级图数据处理能力，图数据库性能基准测试世界纪录保持者。',
  ...
  'demo.example.solution0':
    '图计算技术正是解决这种复杂金融关系的有效手段。通过构建体系化监控，进一步识别风险全貌，显著提高遏制洗钱、欺诈等恶意金融行为的精准性。 TuGraph 同时支持 OLTP 和 OLAP，其高吞吐、低延迟的产品特性，能够快速识别和防范金融风险行为，满足日趋严格的防控要求。',
  'demo.example.solution1':
    '采用图数据模型代替传统关系型数据，每个顶点代表一个设备，每条边代表一条电力线， 可以更直观的呈现电力设备及网络。 通过TuGraph对图数据进行统一存储、管理和计算，其强大的查询语言、多层次灵活的 API，能够实时追踪电网运行状态，支持频繁且复杂的实时数据查询和更加高效的电网 模拟计算。',
  'demo.example.solution2':
    '团伙在申请信用卡时，通常会重复使用一些特定信息。在TuGraph上构建信用卡申请关联图谱，通过产内置的社区发现等算法挖掘异常申请信息，进而锁定可疑群体团体。',
  'demo.example.profit0':
    '通过图计算技术，2020全年反洗钱风险审理分析效率提升 90%，风险识别能力提升了 9.4 倍， 有效的保护了支付宝百万亿交易规模的资金安全。',

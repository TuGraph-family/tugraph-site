export default {
  ...
  'platform.slog': `灵活易用的一站式图平台图的全生命周期管理`,
  'platform.mobileSlog':
    '灵活易用的一站式图平台，图的全生命周期管理，离在线一体化运维管理',
  'platform.slog1': '离在线一体化运维管理',
  'platform.descTitle':
    '灵活易用的一站式图平台，图的全生命周期管理，离在线一体化运维管理',
  'platform.descText':
    '蚂蚁自研的一站式图平台，覆盖图探索、图研发、图服务、运维管理的完整链路，实现图的全生命周期管理，以无代码的交互方式沉淀业务经验。支持离线、实时、关系数据库等多种类型的数据源，支持快速图构建、图查询、图分析，提供图数据库引擎、图分析引擎的研发控制台。',
  ...
  'db.descTitle':
    '为企业级客户服务而专研，高可用、高稳定、高性能的大规模分布式图数据库',
  'db.descText':
    '企业级分布式图数据库：蚂蚁自研的金融级分布式图数据库，万亿图数据处理能力，千万顶点/秒的高吞吐率和低延迟，轻松应对不断变化的业务数据。支持ACID事务处理，保障数据一致性，金融级高可靠。用于实时图查询，面向有海量并发、需要实时更新、数据可靠性要求高的场景，支持强弱一致性。',
  'db.proAdvant': '产品优势',
  'db.advantTitle': '高可用',
  'db.advantDesc':
